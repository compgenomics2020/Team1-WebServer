...
class Diamond(Aligner):
    ...
    version = None
    NAME = "Diamond"
    VERSION_CHECK_CMD = ["/home/projects/group-a/bin/Diamond", "version"]
    VERSION_PATTERN = r"diamond v(.+)"

    def format_db_command(self, db_fasta_file, db_name):
        return ["/home/projects/group-a/bin/Diamond", "makedb", "--in", db_fasta_file, "--db", db_name]

    def get_command(self, query_file, db_name, result_file):
        return ["/home/projects/group-a/bin/Diamond", "blastp", "--query", query_file, "--db", db_name, "--out", result_file, "--threads 1 --max-hsps 1 --max-target-seqs 1"]

    def get_self_alignment_command(self, query_file, db_name, result_file):
        return ["/home/projects/group-a/bin/Diamond", "blastp", "--query", query_file, "--db", db_name, "--out", result_file, "--threads 1 --max-hsps 1 --max-target-seqs 100"]
